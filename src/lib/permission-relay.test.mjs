...
describe('timeout continuation flow still works', () => {
  it('buildClaudeArgs still produces --print and uses CLAUDE_PERMISSION_MODE', async () => {
    const { buildClaudeArgs } = await import('./claude-agent.mjs');
    const args = buildClaudeArgs({ sessionId: 't', resume: false });
    expect(args).toContain('--print');
    expect(args).toContain('stream-json');
    // The --permission-mode flag carries the Claude CLI value (default: 'acceptEdits').
